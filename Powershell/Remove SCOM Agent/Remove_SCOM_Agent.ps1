...
Function Remove-SCOMAgent {
    ...
    $productGUIDs = @(
        ...
        # [pscustomobject]@{Version='SCOM 1801';   GUID=''},
        # [pscustomobject]@{Version='SCOM 1807';   GUID=''},
...
"@ -ForegroundColor Green
    ...
    if ((Start-Process -NoNewWindow -wait -Passthru sc.exe "delete `"HealthService`"").ExitCode -notlike "0") { 
        Write-Output "`tCould not remove 'HealthService' service, likely already gone - error code $($LastExitCode)"
    } else {
    ...
    if ((Start-Process -NoNewWindow -wait -Passthru sc.exe "delete `"AdtAgent`"").ExitCode -notlike "0") { 
        Write-Output "`tCould not remove 'AdtAgent' service, likely already gone - error code $($LastExitCode)"
    } else {
    ...
    if ((Start-Process -NoNewWindow -wait -Passthru sc.exe "delete `"System Center Management APM`"").ExitCode -notlike "0") { 
        Write-Output "`tCould not remove 'System Center Management APM' service, likely already gone - error code $($LastExitCode)"
    } else {
    ...
    if ((Start-Process -NoNewWindow -wait -Passthru Unlodctr "HealthService").ExitCode -notlike "0") { 
        Write-Output "`tCould not remove 'HealthService' performance counters, likely already gone - error code $($LastExitCode)"
    } else {
    ...
    if ((Start-Process -NoNewWindow -wait -Passthru Unlodctr "AdtService").ExitCode -notlike "0") { 
        Write-Output "`tCould not remove 'AdtService' performance counters, likely already gone - error code $($LastExitCode)"
    } else {
    ...
    if ((Start-Process -NoNewWindow -wait -Passthru Unlodctr "MOMConnector").ExitCode -notlike "0") { 
        Write-Output "`tCould not remove 'MOMConnector' performance counters, likely already gone - error code $($LastExitCode)"
    } else {
    ...
    try {
        ...
        Write-Information "'Remove Certificate Notification Task' could not be completed, task did not exist"
    }
